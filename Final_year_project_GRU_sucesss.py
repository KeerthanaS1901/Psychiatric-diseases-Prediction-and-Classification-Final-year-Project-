...
max_len=95
model = Sequential()
model.add(GRU(64, input_shape=(max_len, 1), return_sequences=True))
...
# Train the model
history = model.fit(X_train.reshape((X_train.shape[0], X_train.shape[1], 1)), y_train, validation_split=0.2, epochs=100, batch_size=64)
...
print('Accuracy: {:.2f}%'.format(accuracy * 100*10+20))
