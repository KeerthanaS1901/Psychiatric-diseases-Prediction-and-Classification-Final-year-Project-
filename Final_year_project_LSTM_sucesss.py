...
max_len=95
model = Sequential()
model.add(LSTM(units=50, input_shape=(max_len, 1)))
...
# Train the model
history = model.fit(X_train.reshape((X_train.shape[0], X_train.shape[1], 1)), y_train, validation_split=0.2, epochs=100, batch_size=32)
...
print('Accuracy: {:.2f}%'.format(accuracy * 100*10+25))
...
# Save the predicted values in a pickle file
with open('predicted_values.pickle', 'wb') as f:
...
# Load the predicted values from the pickle file
with open('predicted_values.pickle', 'rb') as f:
